...
class CloudflareSpeedTest(tk.Tk):
    ...
    def load_config(self):
        ...
        self.filepath = config_data.get('filepath', '')
        return config_data
    ...
    def init_components(self):
        ...
        self.expected_servers.insert(0, self.config['expected_servers'])
        self.expected_servers.pack(side=tk.LEFT, padx=5)

        self.use_tls = tk.BooleanVar(value=self.config.getboolean('use_tls'))
        ttk.Checkbutton(control_frame, text="启用TLS", variable=self.use_tls).pack(side=tk.LEFT, padx=5)

        self.ip_type = tk.StringVar(value=self.config['ip_type'])
        ...
        self.bandwidth.insert(0, self.config['bandwidth'])
        ...
        self.timeout.insert(0, self.config['timeout'])
        ...
        self.download_time.insert(0, self.config['download_time'])
    ...
    def test_process(self):
        ...
        while self.testing:
            ...
            if len(qualified_servers) >= self.expected_servers_count:
                ...
                self.save_results(qualified_servers)
                return

    def save_results(self, qualified_servers: Optional[List[SpeedTestResult]] = None):
        ...
        if not self.filepath:
            ...
            filepath = os.path.join(self.filepath, 'find.txt')
        try:
            with open(filepath, 'a', encoding='utf-8') as f:
                ...
                for result in qualified_servers:
                    ...
                    ports = self.TLS_PORTS if self.use_tls.get() else self.NON_TLS_PORTS
    ...
    def save_config(self):
        ...
        config = configparser.ConfigParser()
        config['DEFAULT'] = {
            'expected_servers': self.expected_servers.get(),
            'use_tls': str(self.use_tls.get()),
            'ip_type': self.ip_type.get(),
            'bandwidth': self.bandwidth.get(),
            'timeout': self.timeout.get(),
            'download_time': self.download_time.get(),
            'filepath': self.filepath
        }
        with open('config.ini', 'w') as configfile:
            config.write(configfile)

    def _write_result_to_file(self, f, result: SpeedTestResult):
